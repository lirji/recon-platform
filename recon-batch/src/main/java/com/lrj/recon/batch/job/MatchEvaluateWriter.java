 ...
 *   <li><b>驱动处理链 (M5)</b>: 对有差组驱动 {@link DiscrepancyHandlerChain} —— TRANSACTIONAL handler
 *       (LedgerHandler 审计 / ReversalSuggestionHandler 冲正建议) 在<b>同 chunk 事务内</b>写库;
 *       EXTERNAL handler (AlertHandler) <b>只 insert alert_outbox 不发送</b>, 外部投递交批后 alertRelayStep 中继
 *       (ADR-10, 外部副作用绝不在可重试 chunk 事务内直发); 全部 handler 幂等 (handlerId+fingerprint), chunk 重试不重复;</li>
 ...
 * </ol>
 * 均同一 chunk 事务提交; discrepancy / handler 产物 / partial 同源同批, 天然一致。消除 M2 report Step 的二次全量重放扫描。
 *
...
public class MatchEvaluateWriter implements ItemWriter<EvaluatedGroup> {
    ...
    private final DiscrepancyHandlerChain handlerChain;
    ...
    /** 处理链上下文: 自动处理链的 operator 记为 "system" (人工核销走 ManualClearingService, 另有独立在线事务)。 */
    private final HandlerContext handlerCtx;
    ...
    public MatchEvaluateWriter(DiscrepancyRepository discrepancies, ConservationPartialRepository partials,
                               DiscrepancyHandlerChain handlerChain, PartitionFailureGate failureGate,
                               String runId, String segmentId, int bucket, int subIndex, int subFanout) {
        ...
        this.handlerCtx = new HandlerContext(runId, "system");
    }
    ...
    public void write(Chunk<? extends EvaluatedGroup> chunk) {
        ...
        for (EvaluatedGroup eg : chunk.getItems()) {
            ...
            if (eg.hasDiscrepancy()) {
                ...
                handlerChain.handle(eg.discrepancy(), handlerCtx);     // M5 处理链: 冲正建议/审计(同事务) + 告警入 outbox(不发)
            }
        }
