...
/**
 * B4 Phase 3a · 启动期把内置 MARKETING_3WAY 的声明式定义 seed 进配置存储(幂等:仅当缺失时写入),
...
public class ScenarioDefinitionSeeder implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ScenarioDefinition seed = MarketingThreeWayDefinition.seed();
        if (store.find(seed.code()).isPresent()) {
            return; // 幂等:已存在(可能被管理台改过),不覆盖
        }
        store.save(seed, true);
        log.info("[scenario] seeded built-in definition code={} segments={}", seed.code(), seed.segments().size());
    }
}
