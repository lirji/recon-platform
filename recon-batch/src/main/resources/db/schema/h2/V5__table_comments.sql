-- ============================================================================
-- V5 · 表/列注释 (H2 方言, 测试库)。由 Flyway {vendor}=h2 目录装配 (仅 H2 库执行)。
...
COMMENT ON COLUMN recon_record.match_key           IS 'sort-merge 勾兑键(可空); H2 默认 Unicode 码点序 (V3 no-op) 与 Java 码元序对齐(BMP 内); 尾随空白已标准化 trim';
