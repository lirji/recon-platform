...
**分桶与勾兑（易踩坑）**：`bucket = floorMod(hash(group_key), N)`（桶键=group_key，`Bucketing`）。**不变式：match_key 必须是 group_key 的细分**（M2/M3 为 `match_key==group_key`），`Bucketing.assertIdentityRefine` 装配期 fail-fast——否则同发放单跨桶分裂产生假阳性 GROUP_SUM。`match_key` 可空且是勾兑键：游标排序用**可移植 `ORDER BY (match_key IS NULL), match_key`**（消除 MySQL NULLS-first vs PG NULLS-last），且 **null 键记录逐条路由为单边组、绝不进 `SortMergeJoiner`**（joiner 拒 null 键会抛异常）。
